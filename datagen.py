...
IMAGE_W = 144
IMAGE_H = 192
IMAGE_DIR = 'data/mnistasjpg/trainingSample'
...
for root, subdirs, files in os.walk(IMAGE_DIR):
    for file in files:
        ...
        if len(img.shape) != 3 or img.shape[2] != 3:
            assert(False)
        #hack?
        img = cv2.resize(img, dsize=(151, 197), interpolation=cv2.INTER_CUBIC)
        ...
        for i in xrange(SAMPLES_PER_IMG):
            ...
            if ix < SAMPLES_PER_IMG*16:
                ...
                print path
            ix += 1
            y_data[ix] = flip(y_data[ix - 1], axis=2)
            x_data[ix] = flip(x_data[ix - 1], axis=2)
            ix += 1
